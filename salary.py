...
# Загрузка данных из Excel (предполагается, что файл уже загружен)
# Если файл загружается вручную, используйте:
# df = pd.read_excel('Зарплаты.xlsx', sheet_name='Зарплаты')

# Создадим тестовые данные на основе вашей структуры
data = {
...
# Транспонируем данные по отраслям для удобства
...
print(growth.mean())
'''
Во всех отраслях наблюдается рост номинальных зарплат, но:
...
1) Добывающая промышленность демонстрирует наибольшую устойчивость к инфляции, зарплаты в этой отрасли самые большие
2) Образование  и Строительство наиболее уязвимы к инфляционным процессам
3) В последние года Строительство демонстрирует самые большие темпы роста из всех 3 отраслей'''
